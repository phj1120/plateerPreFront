    ...
    <v-col cols="12">
      <div>
        <v-form ref="form" v-model="rollingInfo.valid" lazy-validation class="formLength" enctype="multipart/form-data">
          <div>
            <v-card>
              <v-text-field v-model="rollingInfo.target" :counter="10" :rules="rollingInfo.nameRules" label="TARGET"
                            required></v-text-field>
              <v-text-field v-model="rollingInfo.title" :rules="rollingInfo.titleRules" label="TITLE"
                            required></v-text-field>
              <v-file-input multiple type="file" counter label="File input" v-model="files"
                            @change="handleChangeFile" @click:clear="handleDeleteFile"></v-file-input>
              <hr>
              <v-img v-for="(item,i) in uploadimageurl" :key="i" :src="item.url" contain height="40vh" width="40vw"
